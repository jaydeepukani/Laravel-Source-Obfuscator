...
class SourceEncryptCommand extends Command
{
    ...
    public function handle()
    {
        ...
        if (empty($this->option('source'))) {
            $sources = config('source-obfuscator.source', ['app', 'database', 'routes']);
        } else {
            $sources = $this->option('source');
            $sources = explode(',', $sources);
        }
        if (empty($this->option('destination'))) {
            $destination = config('source-obfuscator.destination', 'encrypted');
        } else {
            $destination = $this->option('destination');
        }
        if (empty($this->option('keylength'))) {
            $keyLength = config('source-obfuscator.key_length', 6);
        } else {
